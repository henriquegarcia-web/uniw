...
const maskHandlers: MaskHandlers = {
  ...
  // Formata string como CNPJ: 00.000.000/0001-00
  cnpj: (value) => {
    if (!value) return ''
    return value
      .replace(/\D/g, '')
  ...
  // Formata string como RG: 00.000.000-0
  rg: (value) => {
    if (!value) return ''
    return value
      .replace(/\D/g, '')
  ...
  // Formata Código de Cupom/Voucher
  couponCode: (value) => {
    if (!value) return ''
    return value
      .toUpperCase()
      .replace(/[^A-Z0-9-]/g, '') // Permite letras, números e hífen
      .slice(0, 20) // Limita o tamanho do cupom
  },
}
